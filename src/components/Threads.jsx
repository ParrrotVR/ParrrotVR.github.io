...
void main() {
  ...
  for (int i = 0; i < u_line_count; i++) {
    ...
    line_strength *= 1.0 - lineFn(uv, u_line_width * pixel(1.0, iResolution.xy) * (1.0 - p), p, uMouse, iTime, uAmplitude, uDistance);
  }
  float value = 1.0 - line_strength;
  gl_FragColor = vec4(uColor * value, value);
}`;

export default function Threads({ color = [0.76, 1, 0.28], amplitude = 1.15, distance = 0.25 }) {
  ...
  return <div className="threads" ref={containerRef} aria-hidden="true" />;
}
